...
module.exports = function(app, watchFile, conf = {}) {
  ...
  watcher.on('all', function(event, path) {
    if (event === 'change' || event === 'add') {
      try {
        ...
        console.log(
          `${` Done: `.green_b.black} Hot Mocker ${
            watchFile.replace(process.cwd(), '').green
          } file replacement success!`
        )
      } catch (ex) {
        console.error(
          `${` Failed: `.red_b.black} Hot Mocker ${
            watchFile.replace(process.cwd(), '').red
  ...
  app.all('/*', function(req, res, next) {
    ...
    const containMockURL = Object.keys(proxy).filter(function(kname) {
      ...
      return new RegExp(
        '^' + kname.replace(/(:\w*)[^/]/gi, '(\\w*)[^/]') + replaceStr
      ).test(proxyURL)
    })
    ...
    if (proxy[proxyURL] || (containMockURL && containMockURL.length > 0)) {
      ...
      proxyNames.length > 0 &&
      (proxyMatch.length > 0 || proxyFuzzyMatch.length > 0)
    ) {
      const currentProxy =
        proxyConf[proxyMatch.length > 0 ? proxyMatch[0] : proxyFuzzyMatch[0]]
